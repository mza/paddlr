class TimingFactory
  ...
  def self.refresh_listing_overnight(*args)
    options = args.extract_options!    
    doc = options[:js]
    @boat = nil
    doc.each do |line|
      if line.match /^OvernightPs/
        details = line.split("=")
        ...
        unless key == "i"
          ...
          unless @boat.nil?
            ...
            case key
              when "4": timing(:time => value, :location => "Devizes", :boat => @boat)
              when "5": timing(:time => value, :location => "Pewsey", :boat => @boat)
              when "6": timing(:time => value, :location => "Hungerford", :boat => @boat)
              when "7": timing(:time => value, :location => "Newbury", :boat => @boat)
              when "8": timing(:time => value, :location => "Aldermaston", :boat => @boat)
              when "9": timing(:time => value, :location => "Reading", :boat => @boat)
              when "10": timing(:time => value, :location => "Marsh", :boat => @boat)              
              when "11": timing(:time => value, :location => "Marlow", :boat => @boat)              
              when "12": timing(:time => value, :location => "Bray", :boat => @boat)              
